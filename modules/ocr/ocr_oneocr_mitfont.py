...
@register_OCR("one_ocr_mitfont")
class OCROneAPIFont(OCRBase):
    params = {
    ...
    def _ocr_blk_list(self, img: np.ndarray, blk_list: list[TextBlock], *args, **kwargs):
        ...
        regions, textblk_lst_indices = collect_textblock_regions(img, blk_list, self.model.text_height, self.model.maxwidth, False, None)
        self.model(blk_list, regions, textblk_lst_indices, chunk_size=16)
    ...
    def updateParam(self, key: str, content):
        ...
        if key == 'device' and self.device != content and self.model is not None:
            self.model.to(content)
        if self.debug_mode and self.logger:
